...
Sub ImportAndGenerate()
    ...
    ' ピボットテーブルを生成
    pivotData = CreatePivotTable(csvData)

    If IsEmpty(pivotData) Then
        MsgBox "ピボットテーブルの生成に失敗しました", vbCritical
        Exit Sub
    End If

    ' データをシートに書き込み
    rowCount = UBound(pivotData, 1) - LBound(pivotData, 1) + 1
    colCount = UBound(pivotData, 2) - LBound(pivotData, 2) + 1

    ws.Range("A1").Resize(rowCount, colCount).Value = pivotData

    ' 書式設定
    FormatSheet ws, rowCount, colCount

    MsgBox "インポートとピボット生成が完了しました。" & vbCrLf & _
...
' ピボットテーブルを生成する関数
Private Function CreatePivotTable(csvData As Variant) As Variant
    Dim companies As New Collection
    Dim months As New Collection
    Dim pivotDict As Object
    Dim i As Long
    Dim monthCol As String
    Dim companyCol As String
    Dim amountCol As String
    Dim key As String
    ...
    ' ヘッダー行をスキップして、会社名と月のユニークなリストを作成
    For i = 2 To UBound(csvData, 1)
        monthCol = Trim(csvData(i, 1))
        companyCol = Trim(csvData(i, 2))
        amountCol = csvData(i, 3)

        ' 会社名を追加（重複チェック）
        On Error Resume Next
        companies.Add companyCol, companyCol
        On Error GoTo ErrorHandler

        ' 月を追加（重複チェック）
        On Error Resume Next
        months.Add monthCol, monthCol
        On Error GoTo ErrorHandler

        ' Dictionaryにデータを格納
        key = companyCol & "|" & monthCol
        pivotDict(key) = amountCol
    Next i

    ' 結果配列を初期化（ヘッダー行 + データ行）
    ReDim result(1 To companies.Count + 1, 1 To months.Count + 1)

    ' ヘッダー行を作成
    result(1, 1) = "会社名"
    For i = 1 To months.Count
        result(1, i + 1) = months(i)
    Next i

    ' データ行を作成
    For companyIndex = 1 To companies.Count
        companyName = companies(companyIndex)
        result(companyIndex + 1, 1) = companyName

        For monthIndex = 1 To months.Count
            monthName = months(monthIndex)
            key = companyName & "|" & monthName

            If pivotDict.Exists(key) Then
                result(companyIndex + 1, monthIndex + 1) = pivotDict(key)
            Else
                result(companyIndex + 1, monthIndex + 1) = 0
            End If
        Next monthIndex
    Next companyIndex
...
ErrorHandler:
    MsgBox "ピボットテーブルの生成中にエラーが発生しました: " & Err.Description, vbCritical
    CreatePivotTable = Empty
End Function
